...
Or download the matching archive for your platform from GitHub Releases, extract it, and put `ccbf` on your `PATH`.
...
3. GitHub Actions builds macOS arm64/x64 and Linux x64 binaries, creates a GitHub Release, and uploads checksums plus a generated Homebrew formula.
4. The workflow also updates `Formula/ccbf.rb` on `main`, so the Homebrew install URL always points at the latest release.
...
- `Formula/ccbf.rb` is generated by the release workflow after the first tagged release
